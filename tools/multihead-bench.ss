...
;; multihead-bench.ss -- the acceptance measurement for attached editing.
;;
...
;;
;;   scheme-script tools/multihead-bench.ss   (from the repository root)
;;
...
(eval
  '(begin
     ...
     ;; The 50k-line file: every library twice, as in the review.
     (define (write-big-file!)
       (call-with-output-file big-file
         (lambda (out)
           (do ([pass 0 (+ pass 1)]) ((= pass 2))
             (for-each (lambda (name)
                         (when (string:suffix? ".e" name)
                           (display (call-with-input-file (string-append "lib/" name) get-string-all) out)))
                       (list-sort string<? (directory-list "lib")))))
         'replace))
     ...
     (define (run!)
       ...
       (let ([lines (count-lines big-file)]
         ...
         (dynamic-wind void
           (lambda ()
             (let* ([vt-file (string-append here "/lib/vt.e")]
                    ...
                    ;; a second head showing vt.e while the first types
                    [watch
               ...
               (printf "| Head I/O bytes per ~a typed keys, vt.e | ~a | ~a |\n" keys (kb (cadr solo-vt)) (kb (cadr head-vt)))
