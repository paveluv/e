...
;; protocol bootstrap shutdown final-shutdown session recovery restart force help.
(import (chezscheme))
...
(eval
  '(begin
     ...
     (define (bootstrap-scenarios!)
       ;; Automatic bootstrap needs only the installation: a stopped base
       ;; record names it for the heads' launch commands.
       (set! test-base (fixture:start! root base-directory))
       (fixture:stop! test-base)
       (dynamic-wind void
         (lambda ()
     ...
     ;; One process may run every group in order, or the groups named on the
     ;; command line so a runner spreads them over processes.
