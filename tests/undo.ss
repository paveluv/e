...
(eval
  '(begin
     ...
     (mode:choose! formatted "undo-format")
     (head:buffer-trailing-set! formatted #f)
     (register-formatter! "undo-format" (lambda (b from to) '("BASE" "OTHER")))
     (format-buffer!)
     ...
     (register-indenter! "undo-format" (lambda (b from to) '(2 4)))
     (indent-buffer!)
     ...
     (mode:choose! foreign-fact "undo-format")
     (format-buffer!)
     (store:set-property! bot foreign-fact-id 'trailing #f)
