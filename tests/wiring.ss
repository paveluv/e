  ...
  '(begin
     ...
     (check 'filter-backspace-and-distinct-header-with-a-single-match
       (list (screen-has? 0 "Filter: 日本語") (screen-has? 1 "Buffer")
             ...
             (string:prefix? "  Type to filter  ↑↓ choose  Enter open  Esc return  M-m modified  C-u clear" (buffers-bar 0)))
       '(#t #t #t #f #t #t))
     ...
     (define picker-sort-cases
       '(("Lines" (beta alpha gamma) (gamma alpha beta))
         ...
         ("Modified" (beta alpha gamma) (alpha gamma beta))
         ("Read-only" (alpha beta gamma) (beta gamma alpha))))
     ...
     (define picker-compound-cases
       '(("Modified" ("Modified ↑1") (beta alpha gamma))
         ("Read-only" ("Modified ↑1" "Read-only ↑2") (beta alpha gamma))
         ("Lines" ("Modified ↑1" "Read-only ↑2" "Lines ↑3") (beta alpha gamma))
         ("Lines" ("Modified ↑1" "Read-only ↑2" "Lines ↓3") (beta gamma alpha))
         ("Modified" ("Modified ↓1" "Read-only ↑2" "Lines ↓3") (gamma alpha beta))
         ("Modified" ("Read-only ↑1" "Lines ↓2") (gamma alpha beta))
         ("Modified" ("Read-only ↑1" "Lines ↓2" "Modified ↑3") (gamma alpha beta))
         ("Lines" ("Read-only ↑1" "Modified ↑2") (beta alpha gamma))
         ("Read-only" ("Read-only ↓1" "Modified ↑2") (beta alpha gamma))
         ("Read-only" ("Modified ↑1") (beta alpha gamma))
         ("Modified" ("Modified ↓1") (alpha gamma beta))
     ...
     (define narrow-modified
       (list (picker-face "<picker-beta>" "3") (screen-line (car (find-cell "<picker-beta>")))))
     ...
     (check 'narrow-table-keeps-the-filename-tail-without-wrapping
       (list (visible? "…") (visible? "beta.ss")
             ...
             (car narrow-modified) (picker-face "<picker-beta>" "3")
             (equal? (cadr narrow-modified) (screen-line (car (find-cell "<picker-beta>")))))
       '(#t #t (5 #f) #t #f #t))
     (resize! 6 32)
     (press! "\x1b;[F")
     (check 'filter-and-headings-stay-visible-while-browsing-a-short-pane
       (list (screen-has? 0 "Filter: picker-") (screen-has? 1 "Buffer")
             (picker-face "<picker-gamma>" "1")) '(#t #t #t))
     ...
     (check 'two-list-windows-keep-independent-identities-through-a-sort
       (list (read-editor
             ...
             (string:prefix? "  Type to filter" (buffers-bar 0)) (quiet-buffers-bar? 1))
       '((#t #t) #t #t))
