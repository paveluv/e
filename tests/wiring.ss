...
;; a foreign actor's store edit appears on the user's screen. Drives an
;; in-memory head under a PTY so callbacks can deliberately interleave inside
;; the store. Real process separation is covered by wire.ss and interactive.ss.
...
(eval
  '(begin
       ...
       (let* ([line (screen-line n)]
         ...
         (let scan ([i 0])
           (cond [(> (+ i len) (string-length line)) #f]
                 ...
                 [else (scan (+ i 1))]))))
     (define (buffers-bar index)
       (let* ([prefix (format "~a▏<buffers>" index)]
     ...
     (check 'both-switch-shortcuts-keep-the-previous-document-default
       (picker-sequence (lambda (keys) (press! (string-append keys "\r")) (picker-state))
     ...
     (let* ([filter-row (screen-line 0)]
       ...
       (press! "\x0c;")
       (check 'buffer-heading-and-candidate-theme-reports-repaint-without-changing-the-filter
         (list (> queries-before 0) (= (background-queries) (+ queries-before 1))
           (picker-sequence
             (lambda (report)
               (press! report)
               (hover! (find-cell "Modified"))
               (let ([hovered (picker-heading-hover)] [candidate (picker-style "<picker-alpha")])
                 (hover! (find-cell "<picker-alpha"))
                 (let ([pointed (picker-style "<picker-alpha")])
                   (hover! (find-cell "Filter:"))
                   (list (picker-style "Buffer") candidate pointed
         ...
         '(#t #t (("0;38;5;236;48;5;253" "0;1;48;2;226;235;250" "0;1;48;2;226;235;250;4:4;58;5;248" #t)
                  ("0;38;5;252;48;5;240" "0;1;48;2;28;40;60" "0;1;48;2;28;40;60;4:4;58;5;242" #t)
                  ("0;38;5;236;48;5;253" "0;1;48;2;226;235;250" "0;1;48;2;226;235;250;4:4;58;5;248" #t)
                  ("0;38;5;236;48;5;253" "0;1;48;2;226;235;250" "0;1;48;2;226;235;250;4:4;58;5;248" #t)
                  ("0;38;5;252;48;5;240" "0;1;48;2;28;40;60" "0;1;48;2;28;40;60;4:4;58;5;242" #t)))))
     ...
     (press! "\x18;bpicker-")
     (define (picker-order names) (map (lambda (name) (format "<picker-~a>" name)) names))
     (define picker-all (picker-order '(alpha beta gamma)))
     ...
     (check 'hover-has-one-candidate-without-a-text-cursor-or-mark
       (list (map hover-face '("<picker-alpha>" "<picker-beta>" "<picker-gamma>"))
             (map (lambda (name) (picker-face name "48")) '("<picker-alpha>" "<picker-beta>" "<picker-gamma>"))
     ...
     (check 'arrows-continue-from-hover-and-clear-its-old-emphasis
       (map (lambda (name) (append (hover-face name) (list (picker-face name "48"))))
         '("<picker-alpha>" "<picker-beta>" "<picker-gamma>"))
       '((#f #f #f) (#f #f #f) (#t #f #t)))
     (wheel! (find-cell "<picker-gamma>") 'up)
     (check 'wheel-browses-without-opening (list (car (picker-state)) (picker-face "<picker-beta>" "1"))
       '("<buffers>" #t))
     (resize! 8 26)
     ;; Flags are omitted here; saving must still remove italic emphasis
     ;; when the printed row is unchanged.
     (read-editor '(begin (head:buffer-modified-set! (buffer "<picker-beta>") #t) #t))
     (define narrow-modified
       (list (picker-face "beta.ss" "3") (screen-line (car (find-cell "beta.ss")))))
     (read-editor '(begin (head:buffer-modified-set! (buffer "<picker-beta>") #f) #t))
     ...
     ;; The filesystem app shares the picker input and PTY helpers. One tree
     ;; exercises navigation, collapsed/expanded groups, metadata, opening,
     ;; cancellation and independently formatted panes.
     ...
     (define (files-open!)
       (read-editor `(begin (file-view:expansion-limit 2) (file-view:open! ,files-root) #t))
       (files-settle!))
     (define (files-settle!)
       (let wait ([left 100])
         (pump! 50)
         (when (visible? "Searching…")
           (when (zero? left) (error 'files "scan did not settle" (vector->list (vt:emulator-screen mirror))))
           (wait (- left 1)))))
     (define (files-labels)
       (read-editor
     ...
     (define (path-table? name)
       ;; Read only the live table, not the same filename inside its prompt.
       (let ([input (find-cell "Create file:")])
         (and input (exists (lambda (row) (screen-has? row name))
                      (map (lambda (i) (+ i 3)) (iota (max 0 (- (car input) 3))))))))
     (files-open!)
     (press! "\x0c;\x18;\x06;") ; C-l redraws; C-x C-f opens Files
     (files-settle!)
     (check 'files-list-has-only-subdirectories-and-files-with-a-full-directory-path
       (let ([labels (files-labels)])
         (list (list-head labels 4)
               ...
               (map (lambda (name) (picker-face name "48")) '("empty/" "large/" "small/"))
               (read-editor '(list (head:buffer-store-id (current-buffer))
                                   ...
                                   (eq? (keymap:binding "C-x C-f") file-view:open!)
                                   (keymap:binding "C-x f") (keymap:command-keys 'find-file!!)))))
       (list '("empty/" "large/" "small/" "a 日本語 long (name).txt")
         (string-append "Directory: " files-root "/")
         #t #f '((#t #f) (#f #f) (#f #f)) '(#t #f #f) '(#f #f #f #t #f ())))
     (press! "\x1b;[B\x1b;[BONly") ; begin on small/, whose descendant will match
     (files-settle!)
     (check 'files-single-recursive-match-is-the-default
       (list (visible? "small/nested/needle-")
             (read-editor '(string:prefix? "small/nested/needle-only.txt" (buffer-line (current-buffer) (car (point)))))) '(#t #t))
     (press! "\r")
     (define files-visited
       (read-editor '(begin (goto-point! '(1 . 1)) (insert-text! "!")
     ...
     (files-filter! "apple")
     (press! "\x1b;OQ\x1b;OP")
     (let ([keys (read-editor '(head:buffer-fact (current-buffer) 'file-sorts #f))])
       (hover! (find-cell "Size"))
       (let ([face (hover-face "Size")])
         (click! (find-cell "Size"))
         (let ([descending (visible? "Size¹↓")])
           (click! (find-cell "Size"))
           (check 'files-share-compound-sort-cycle-indicators-and-hover
             (list keys face descending (visible? "Name¹↑")
                   (read-editor '(head:buffer-fact (current-buffer) 'file-sorts #f)))
             '(((1 . #f) (0 . #f)) (#t #t) #t #t ((0 . #f)))))))
     (files-filter! "txt")
     (press! "\x1b;OP\x1b;OP\x1b;OQ") ; disable Name, enable Size ascending
     (check 'files-size-sorting-uses-bytes-within-the-file-group
       (find (lambda (name) (string:suffix? ".txt" name)) (files-labels)) "zeta.txt")
     (files-filter! "")
     (press! "\x1b;.")
     (files-settle!)
     (check 'files-hidden-toggle-reveals-dot-entries (and (member ".dot" (files-labels)) #t) #t)
     (press! "\x1b;.\x00;")
     (files-settle!)
     (check 'files-mark-command-cannot-enable-selection
       (read-editor '(head:buffer-marked (current-buffer))) #f)
     (files-filter! "apple")
     (read-editor '(begin (split-window-right!) (select-window! (window 1)) (split-window-right!)
                          (select-window! (window 2)) (show-buffer! (buffer "*scratch*")) #t))
     (let ([before (read-editor '(head:window-lines (window 0)))])
       (read-editor '(begin (head:transfer-split! (head:layout-parent (head:root) (window 1)) 3) #t))
       (check 'files-refit-only-the-resized-pane-with-shared-query
         (read-editor `(list (equal? ',before (head:window-lines (window 0)))
                             (< (string-length (vector-ref (head:window-lines (window 1)) 2))
     ...
     (check 'files-unicode-filter-opens-the-complete-path
       (read-editor '(head:buffer-file (current-buffer))) (files-path "a 日本語 long (name).txt"))
     (files-open!) (files-filter! "needle") (press! "\x1b;c") (files-settle!)
     (let ([shallow (and (visible? "No matching files") (not (path-table? "needle-one.txt"))
                         (string:prefix? "Filter:   " (screen-line 0))
                         (visible? (string-append "Create file: " (files-path "needle"))))])
       (prompt-input! (files-path "missing/../sm")) (files-settle!)
       (let ([prefix (and (path-table? "small/") (not (path-table? "empty/")))])
         (press! "\t") (files-settle!)
         (let ([descended (and (screen-has? 1 (string-append "Directory: " (files-path "small/")))
                               (path-table? "nested/") (path-table? "needle-one.txt")
                               (not (path-table? "needle-only.txt")))])
           (prompt-input! (files-path "small/needle")) (files-settle!)
           (let ([direct (and (path-table? "needle-one.txt") (not (path-table? "nested/"))
                              (not (path-table? "needle-only.txt")))])
             (press! "\t")
             (check 'files-path-table-follows-completion-without-recursive-filtering
               (list shallow prefix descended direct
                     (visible? (string-append "Create file: " (files-path "small/needle-one.txt"))))
               '(#t #t #t #t #t))
             (press! "\x07;")))))
     (files-open!) (press! "\x1b;c\x1b;OQ\x1b;OQ\x1b;OP\x1b;OQ") ; Name, Size ascending
     (files-settle!) (resize! 10 100)
     (hover! (find-cell "Name"))
     (let ([face (hover-face "Name")])
       (click! (find-cell "Name"))
       (let ([sorted (and (visible? "Name¹↓") (visible? "Size²↑") (screen-has? 3 "small/"))])
         (press! "\t")
         (let ([first (and (visible? "1/2 Tab next") (screen-has? 3 "small/"))])
           (press! "\t")
           (let ([next (and (visible? "2/2") (path-table? "apple.txt") (not (path-table? "small/")))])
             (press! "\t")
             (let ([wrapped (and (visible? "1/2") (screen-has? 3 "small/"))])
               (wheel! (find-cell "small/") 'down)
               (let ([wheel (and (visible? "2/2") (path-table? "apple.txt"))])
                 (press! "\x1b;[5~")
                 (check 'files-live-table-sorts-and-repeated-tab-only-pages-the-candidates
                   (list face sorted first next wrapped wheel (visible? "1/2") (screen-has? 3 "small/"))
                   '((#t #t) #t #t #t #t #t #t #t))))))))
     (resize! 24 100)
     (click! (find-cell "small/")) (files-settle!)
     (let* ([inside (and (screen-has? 1 (string-append "Directory: " (files-path "small/")))
                         (path-table? "needle-one.txt"))]
            [parent (find-cell (string-append (substring files-root 5 (string-length files-root)) "/"))])
       (hover! parent)
       (let ([face (hover-face parent)])
         (click! parent) (files-settle!)
         (hover! (find-cell "apple.txt"))
         (let ([file-face (append (hover-face "apple.txt")
                            (list (picker-face "apple.txt" "48")
                                  (picker-face (cons (car (find-cell "apple.txt"))
                                                 (+ (cdr (find-cell "Entries")) (- (string-length "Entries") 1))) "48")
                                  (picker-face "APPLE.txt" "48")))])
           (click! (find-cell "apple.txt")) (files-settle!)
           (let ([exact (and (path-table? "apple.txt") (not (path-table? "APPLE.txt"))
                             (screen-has? 1 (string-append "Directory: " files-root "/"))
                             (visible? (string-append "Create file: " (files-path "apple.txt"))))])
             (press! "\t\t")
             (check 'files-path-table-clicks-edit-the-path-and-share-breadcrumb-hover
     ...
     (click! (cons 1 (or (string:search (screen-line 1) "small/" 50 (string-length (screen-line 1)))
                         (error 'files "missing side-pane breadcrumb" (screen-line 1)))))
     (files-settle!)
     (check 'files-side-pane-navigation-ends-path-entry-without-reverting-the-click
       (read-editor '(list (head:window-index (selected-window)) (head:buffer-name (current-buffer))
                           (head:buffer-fact (current-buffer) 'directory #f)
                           (head:buffer-named "<create-file>")))
       (list 1 "<files>" (files-path "small") #f))
     (read-editor '(begin (select-window! (window 0)) (delete-other-windows!) #t))
     (files-open!)
     (press! "\x1b;c")
     (prompt-input! (files-path "empty/late.txt")) (files-settle!)
     (call-with-output-file (files-path "empty/late.txt") (lambda (p) (display "new" p)))
     (press! "\r")
     (define appeared
     ...
     (define expired
       (and (not (visible? "already exists")) (visible? "<create-file>")
            (visible? (string-append "Create file: " (files-path "empty/late.txt")))))
     (press! "\t") (files-settle!)
     (let ([fresh (path-table? "late.txt")])
       ...
       (let ([removed (and (visible? "No matching files") (not (path-table? "late.txt")))])
         ...
         (let ([refused (and (visible? (string-append "Create file: " (files-path "empty/") " [directory already exists]"))
           ...
           (let ([edited (and (not (visible? "already exists")) (visible? (files-path "empty/n")))])
             ...
             (check 'files-create-refreshes-completions-and-refuses-existing-paths-with-transient-inline-notices
               (list appeared expired fresh removed refused edited (visible? "Empty directory")
                 (read-editor '(head:buffer-fact (current-buffer) 'directory #f)))
               (list #t #t #t #t #t #t #t (files-path "empty")))))))
     (files-open!)
     (files-filter! "apple")
     (let ([matched (visible? "apple.txt")])
       (press! "\x1b;c")
       (let ([seeded (visible? (string-append "Create file: " (files-path "apple")))])
         (press! "\r")
         (check 'files-explicit-creation-uses-the-filter-even-when-it-matches-existing-files
           (list matched seeded (read-editor '(list (head:buffer-file (current-buffer)) (buffer-clean? (current-buffer))))
                 (file-exists? (files-path "apple")))
           (list #t #t (list (files-path "apple") #t) #t))))
     (files-open!)
     (files-filter! "new/sub/file.txt") (press! "\x1b;c\r")
     (check 'files-path-entry-creates-missing-parents-and-touches-the-file
       (list (read-editor '(list (head:buffer-file (current-buffer))
                                 (map log:datum (reverse (log:entries 'file 3)))))
             (file-directory? (files-path "new/sub")) (file-exists? (files-path "new/sub/file.txt")))
       (list (list (files-path "new/sub/file.txt")
                   (list (string-append "Created directory " (files-path "new/"))
                         (string-append "Created directory " (files-path "new/sub/"))
                         (string-append "Created file " (files-path "new/sub/file.txt")))) #t #t))
     (files-open!)
     (files-filter! "new/dirs/leaf/") (press! "\x1b;c\r") (files-settle!)
     ...
     (let ([errors (picker-sequence
                     (lambda (case)
                       (prompt-input! (files-path (string-append "apple.txt" (car case)))) (press! "\r")
                       (and (visible? (cdr case)) (visible? "Create file:")))
                     '(("" . "[file already exists]") ("/child" . "not a directory") ("/" . "not a directory")))])
       ...
       (check 'files-creation-errors-stay-editable-and-cancelling-has-no-filesystem-effect
         ...
         (list '(#t #t #t) #f "one\ntwo\n" (list (files-path "cancelled/child") ""))))
     (files-open!)
     ...
     (check 'files-kill-and-recreate-rejects-the-old-scan
       (read-editor '(list (head:buffer-fact (current-buffer) 'directory #f) (buffer-line (current-buffer) 0)
                           (head:app-buffer? (current-buffer))))
       (list (files-path "empty") "Filter: " #t))
     (let ([same-view
            (read-editor '(let ([before (current-buffer)])
       ...
       (check 'files-reload-replaces-worker-ownership-and-restores-app-state
         (list same-view (visible? "No matching files")
               (read-editor '(list (head:buffer-fact (current-buffer) 'directory #f)
                                   (buffer-line (current-buffer) 0) (head:app-buffer? (current-buffer)) (file-view:show-hidden))))
         (list #t #t (list (files-path "empty") "Filter: n" #t #t))))
     (read-editor `(begin (show-buffer! (buffer ,files-origin))
                          (kill-buffer! (head:find-tool-buffer "*files*"))
     ...
     (for-each (lambda (name) (delete-file (files-path name))) '("apple" "new/sub/file.txt"))
     ...
     (check 'find-file-candidates-sit-above-input-with-operation-hints
       (list (< (car (find-cell "atlas.txt")) (car (find-cell "Find file: ")))
             (visible? "2 matches") (visible? "↑↓ history  Enter accept  Esc cancel") completion-hover
     ...
     (press! (string-append "\x1b;x\x1b;[200~"
               "parameterize ([prompt:inspector (lambda (s pos) (parameterize ([prompt:validate (lambda (s) (and (string=? s \"inner-\") (prompt:transient \"choose a completion\")))]) (prompt:read! \"Nested: \" (lambda (s) '(\"inner-one\" \"inner-two\")) \"inner-\")))]) (find-file!!)"
               "\x1b;[201~\r"))
     ...
     (check 'nested-prompt-has-its-own-echo-input-and-completion-view
       (list nested-notice nested-expired (visible? "Nested: inner-")
             (hover-face "inner-two") (visible? "<completions>")) '(#t #t #t (#t #t) #t))
     ...
     (for-each
       (lambda (kind)
         (check 'quit-review-setup
           (read-editor
             `(let* ([target (head:new-local-buffer "quit work")]
                     [armed? #t])
                ...
                (list (not (head:buffer-store-id target)) (head:buffer-modified target)))) '(#t #t))
         (send! "\x1b;xquit!!\r")
         (pump! 500)
         ...
         (check 'cancelled-quit-keeps-work-and-the-store-open
           (read-editor
             `(let* ([b (head:buffer-named "<quit work>")]
                     [result (list (buffer-line b 0) (not (head:buffer-store-id b))
                               (head:quitting?))])
     ...
     (read-editor
       '(let ([local (head:new-local-buffer "quit review")])
          ...
          (head:buffer-modified-set! local #t) #t))
     (send! "\x1b;xquit!!\r")
     (pump! 500)
     (send! "v")
     (pump! 700)
     ...
     (read-editor
       '(begin
          (head:buffer-fact-set! (head:buffer-named "<quit review>") 'disposable #t)
          (kill-buffer! (head:buffer-named "<quit review>"))
