...
(eval
  '(begin
     ...
     (define (path-table? name)
       ...
       (let ([input (find-cell "Open/create:")])
         ...
         (let ([descended (and (screen-has? 1 (string-append "Directory: " (files-path "small/")))
           ...
           (let ([direct (and (path-table? "needle-one.txt") (not (path-table? "nested/"))
             ...
             (check 'files-path-table-follows-completion-without-recursive-filtering
               (list shallow prefix descended direct (read-editor '(head:buffer-file (current-buffer))))
               (list #t #t #t #t (files-path "small/needle-one.txt")))))))
     (files-open!) (press! "\x0c;\x1b;OQ\x1b;OQ\x1b;OP\x1b;OQ") ; Name, Size ascending
     ...
     (let ([face (hover-face "Name")])
       ...
       (let ([sorted (and (visible? "Name¹↓") (visible? "Size²↑") (screen-has? 3 "small/"))])
         (press! "\t")
         (let ([first (and (visible? "1/2") (screen-has? 3 "small/"))])
           (press! "\t")
       ...
       (let ([face (hover-face parent)])
         ...
         (let ([file-face (append (hover-face "apple.txt")
           ...
           (let ([exact (and (path-table? "apple.txt") (not (path-table? "APPLE.txt"))
                             ...
                             (visible? (string-append "Open/create: " (files-path "apple.txt"))))])
             (press! "\t\t\r")
             (check 'files-path-table-clicks-edit-the-path-and-share-breadcrumb-hover
               (list inside face file-face exact (read-editor '(head:buffer-file (current-buffer))))
               (list #t '(#t #t) '(#t #t #t #t #f) #t (files-path "apple.txt")))))))
     (files-open!) (press! "\x0c;")
     ...
     (let ([small (and (visible? "Enlarge pane") (cdr (assq 'cursor-visible (vt:emulator-state mirror))))])
       ...
       (let ([restored (and (path-table? "needle-only.txt")
                            (visible? (string-append "Open/create: " (files-path "small/nested/"))))])
         (press! "\x1b;") (files-settle!)
         (check 'files-escape-leaves-path-entry-in-the-visible-directory-after-resize
           (list small restored (visible? "<files>") (visible? "Open/create:")
     ...
     (press! "\x0c;") (prompt-input! (files-path "small/nested/")) (files-settle!)
     ...
     (let ([fresh (path-table? "late.txt")])
       ...
       (let ([removed (and (visible? "No matching files") (not (path-table? "late.txt")))])
         (prompt-input! (files-path "empty")) (press! "\r") (files-settle!)
         (check 'files-path-entry-refreshes-completions-and-can-enter-an-empty-directory
           (list fresh removed (visible? "Empty directory") (read-editor '(head:buffer-fact (current-buffer) 'directory #f)))
           (list #t #t #t (files-path "empty")))))
     (files-open!)
     (files-filter! "apple")
