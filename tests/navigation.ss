       ...
       (lambda (local?)
         (let ([source ((if local? head:new-local-buffer head:new-buffer) "navigation.md")])
           ...
           (let ([entered #f] [once #t])
               ...
               (lambda ()
                 ...
                 (when once
                   ...
                   (set! entered (list (head:buffer-store-id (head:current-buffer))
                                       (head:buffer-line (head:current-buffer) (car (point)))))
             ...
             (check 'source-to-view-keeps-content (head:buffer-line (head:current-buffer) (car (point))) "Middle"))
           (let ([entered #f] [once #t])
             (head:set-repaint-hook!
               (lambda ()
                 (paint:invalidate-screen-cache!)
                 (when once
                   (set! once #f)
                   (set! entered (list (eq? (head:current-buffer) source) (point)))
             ...
             (check 'view-to-source-keeps-content (head:buffer-line source (car (point))) "# Middle"))
           (let ([once #t])
             (head:set-repaint-hook!
               (lambda ()
