...
(elibrary (sys sys)
  ...
  (edoc "Connect to a local socket by a deadline, ten scaled seconds from now by default; an error when no base listens."
        (path file "the socket path")
        (deadline any "a monotonic time")
  ...
  (edoc "A file's metadata without opening it, following a symbolic link when asked: #(kind permissions byte-size modified-ns created-ns), or #f when inaccessible."
        (path string "the path")
