...
| `(request id edit buffer-id basis span lines [context [delta?]])` | Return `(applied receipt)`, `(stale reason)` or `(refused reason)` under the connection's session. Context is `#f` or `(key label . options)`, the options an alist among `(undo . facts)`, `(commit . facts)`, `(expected . review)` and `(labels . alist)`, with `(batch . id)` among the labels naming the edits made together. Expected facts are exact-value pairs or absent symbol keys; mismatch returns `(stale property-changed)`. The receipt is `(revision text changes edit-facts)` with the chain from the basis through the accepted edit and the `modified`/`modified-at` fact pairs captured at that commit. A true `delta?` replaces only the text with `#f`, for a client holding text and facts at the basis. |
...
| `(request id undo-authors buffer-id)`, `history buffer-id [count]`, `blame buffer-id [count]`, `log buffer-id [selector]` | History queries. Blame spans use the plain span codec. `log` returns the retained entries newest first as `(revision actor labels delta origin state)`, the delta as `(span removed inserted)`, narrowed by a selector alist among `count`, `actor`, `batch`, `since`, `until` and `state`, the state `enabled` or `disabled` by a live undo or rewrite. |
| `(request id view buffer-id revisions)` | Return `(text mapping conflicts)`: the buffer's lines with those entries disabled and the rest rebased over their absence, the deltas taking the current text there as data, oldest first, and `(revision . cause)` pairs for the entries left applied, the cause a later overlapping entry's revision or `basis-too-old`. Nothing changes. |
| `(request id rewrite buffer-id revisions)` | Disable those entries for everyone under the connection's session: their inverses become the session's own action and the entries leave their undo groups. Return `(applied revision)`, `(blocked conflicts)` as `view` reports them, or `(refused reason)`. |
| `(request id conflicts buffer-id)` | Return pending `(revision actor labels region mine disk)` conflicts. Disk contains the current text that choosing Disk keeps. |
| `(request id conflict-state buffer-id [cached-revision])` | Return `(text revision conflicts)` from one read, with regions in that text's coordinates. If the optional cached revision equals the returned revision, text is `#f`; the caller reuses its immutable cached text. |
...
| `(request id reference-fetch)`, `reference-page`, `reference-page! name keys basis-list docs`, `reference-lookup name docs`, `reference-entries docs`, `reference-signatures`, `reference-url entry` | Base corpus and private page service. `reference-fetch` starts the download in a base worker on the requester's behalf and replies at once; the fetch's log records are the requester's, presented as progress. `reference-signatures` is the corpus's and registered modules' procedure forms in one piece, `(name form ...)` per name, for a head's completion hints. Fetch/page publication require an all-buffer head; a head addresses its own page. `docs` contains owned module-entry datums scoped to this query. |
