...
and `prompt:reindent`. The echo area is a bordered box of at most `paint:echo-box-width` columns (100 by default), centered on the screen; a narrower screen is the whole box. Messages and prompts wrap inside its borders, row by row, with their text at the left border.
